...
const port = Number(
  process.env.WEB_SMOKE_PORT ?? 3105 + Math.floor(Math.random() * 200),
);
...
const routes = [
  { path: "/", status: 200, contains: "Waiting for live board projection." },
  {
    path: "/live",
    status: 200,
    contains: "Waiting for live board projection.",
  },
...
async function waitUntilReachable(timeoutMs = 90_000) {
  ...
  throw new Error(
    `Web server was not reachable on ${baseUrl} within ${timeoutMs}ms`,
  );
}
